...
export const productIconPaths = {
  ...
  ecommerce: "/product-icons/EsteemedCommerce.svg",
  blogs: "/product-icons/Blog.svg",
  seo: "/product-icons/SEO.svg",
  "website-design": "/product-icons/Design.svg",
  "business-email": "/product-icons/Mail.svg",
};
...
export function getAllowedLookupKeys() {
  return new Set([
    ...
    ...vpsTiers.flatMap(concreteLookupKeysFromTier),
    ...managedHostingTiers.flatMap(concreteLookupKeysFromTier),
    ...commerceTiers.flatMap(concreteLookupKeysFromTier),
    ...wooCommerceTiers.flatMap(concreteLookupKeysFromTier),
    ...drupalCommerceTiers.flatMap(concreteLookupKeysFromTier),
    hostingPageOverage.lookupKey,
    ...agents.map((agent) => agent.lookupKey).filter(Boolean),
    agentFleet.lookupKey,
    ...supportTiers.map((tier) => tier.lookupKey).filter(Boolean),
    ...migrationPackages
