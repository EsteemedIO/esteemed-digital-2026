...
export const SUPPORTED_DOMAIN_TLDS = Object.freeze(Object.keys(TLD_RETAIL_PRICES));
...
function buildXml(action, object, attributes) {
  const attrItems = Object.entries(attributes)
    .map(([k, v]) => `<item key="${escapeXml(k)}">${escapeXml(v)}</item>`)
    .join("\n");
...
export async function lookupDomain(domain) {
  const normalized = normalizeDomainName(domain);
  if (!normalized) throw new Error("Invalid domain format.");
  const result = await request("LOOKUP", "DOMAIN", { domain: normalized });
  return {
    domain: normalized,
...
export async function suggestDomains(searchString, tlds = [".com", ".io", ".net", ".org", ".co"]) {
  ...
  for (const tld of tlds) {
    const domain = normalizeDomainName(searchString.replace(/\.[^.]+$/, "") + tld);
    if (!domain) continue;
...
export function getDomainPrice(tld) {
  ...
  return TLD_RETAIL_PRICES[normalized] ?? null;
}
...
<item key="attributes">
<dt_assoc>
<item key="domain">${escapeXml(normalized)}</item>
<item key="period">${escapeXml(period)}</item>
